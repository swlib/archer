...
abstract class AbstractArcher
{
    ...
    {
        $task = new Task\Async($task_callback, $params, $finish_callback);
    ...
    public static function taskWait(callable $task_callback, ?array $params = null, ?float $timeout = null)
    {
        ...
        $task = new Task\Co($task_callback, $params, $result_receiver);
        if (!Queue::getInstance()->push($task)) {
            throw new Exception\AddNewTaskFailException();
        }
        if (isset($timeout)) {
     ...
     */
    public static function taskDefer(callable $task_callback, ?array $params = null): Task\Defer
    {
        $task = new Task\Defer($task_callback, $params);
        if (!Queue::getInstance()->push($task)) {
            throw new Exception\AddNewTaskFailException();
        }
     ...
     */
    public static function getMultiTask(): MultiTask
    {
        return new MultiTask();
    }

    public static function psr(): \Swlib\Archer
    {
